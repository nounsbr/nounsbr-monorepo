...
task('populate-descriptor', 'Populates the descriptor with color palettes and NounBR parts')
  ...
  .setAction(async ({ nftDescriptor, nounsbrDescriptor }, { ethers, network }) => {
    const options = { gasLimit: network.name === 'hardhat' ? 30000000 : undefined };

    const descriptorFactory = await ethers.getContractFactory('NounsBRDescriptorV2', {
      libraries: {
        NFTDescriptorV2: nftDescriptor,
      },
    });
    const descriptorContract = descriptorFactory.attach(nounsbrDescriptor);
