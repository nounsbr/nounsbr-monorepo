...
export const MintNounsBR = (
  token: NounsBRToken,
  burnNoundersBRTokens = true,
): ((amount: number) => Promise<void>) => {
  return async (amount: number): Promise<void> => {
    for (let i = 0; i < amount; i++) {
      await token.mint();
    }
    if (!burnNoundersBRTokens) return;
...
export const setTotalSupply = async (
  ...
  if (totalSupply < newTotalSupply) {
    ...
    // If NounderBR's reward tokens were minted totalSupply will be more than expected, so run setTotalSupply again to burn extra tokens
